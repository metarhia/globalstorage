...
const { fieldsToHash } = require('./mongodb.utils');

class MongodbCursor extends Cursor {
  constructor(cursor) {
    super();
    this.cursor = cursor;
  }
  ...
  clone() {
    ...
    cursor.parents.push(...this.parents);
  ...
  projection(mapping) {
    const fields = fieldsToHash(mapping);
    fields._id = 1;
