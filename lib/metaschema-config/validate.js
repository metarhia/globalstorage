...
const {
  ...
  default: {
    config: {
      processors: {
        domains: { validateInstance: domainValidator },
      },
    },
  },
  extractDecorator,
...
const validateDomains = (ms, schema, instance, { path = '' } = {}) => {
  const errors = [];
  if (schema.class === 'JSON') {
    try {
      JSON.stringify(instance);
    } catch (error) {
      errors.push(
        new ValidationError('invalidInstance', path, { type: 'JSON' })
      );
    }
  } else if (schema.class === 'Date') {
    if (Number.isNaN(Date.parse(instance))) {
      errors.push(
        new ValidationError('invalidInstance', path, { type: 'Date' })
      );
    }
  } else {
    return domainValidator(ms, schema, instance, { path });
  }
  return errors;
};
...
module.exports = { validate, validateDomains };
