...
const {
  default: { config },
  errors: { SchemaValidationError, ValidationError },
  extractDecorator,
} = require('metaschema');
...
const { validateDomains } = require('./validate');
...
ValidationError.serializers.invalidInstance = ({ property, info: { type } }) =>
  `Property '${property}' is not a valid ${type}`;
...
const domainsProcessors = {
  ...config.processors.domains,
  validateInstance: validateDomains,
};
