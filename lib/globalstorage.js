...
gs.open = function(options, callback) {
  ...
  let Provider = gs.providers[options.provider];
  if (Provider) {
...
gs.connect = function(options, callback) {
  let connection = new gs.Connection(options);
    ...
    if (gs.local) {
      gs.local.get(id, (err, data) => {
        if (data) callback(null, data);
          ...
          let sid = gs.findServer(id);
          let connection = gs.infrastructure.index[sid];
...
function buildIndex(tree) {
  let result = [];
  let parseTree = (index, depth, node) => {
    let isBranch = !!node[0];
    if (isBranch) {
  ...
  let height = Math.ceil(Math.log(result.length) / Math.log(2));
  for (let i = result.length; i >= 0; i--) {
    let depth = Math.ceil(Math.log(i + 1) / Math.log(2));
    for (let j = 1; result[i] && j < 1 << height - depth; j++) {
      if (!result[i + (j << depth)]) {
