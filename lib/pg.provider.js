...
class PostgresProvider extends StorageProvider {
  ...
  set(obj, callback) {
    ...
    this.getCategoryById(obj.Id, (err, category) => {
      ...
      const error = this.gs.schema.validate(category, obj);
      if (error) {
        callback(new GSError(
        ...
        metasync.sequential([
          ...
          (ctx, cb) => {
            client.query(
              ...
              ' "Category" = (SELECT "Id" FROM "Category" WHERE "Name" = $2)' +
              ' WHERE "Id" = $1',
              [ctx.id, category],
              err => {
                cb(err);
  ...
  update(category, query, patch, callback) {
    const error = this.gs.schema.validate(category, patch);
    if (error) {
      callback(new GSError(
        errorCodes.INVALID_SCHEMA,
        `Invalid schema provided: ${error.toString()}`
...
module.exports = { PostgresProvider, recreateIdTrigger };
