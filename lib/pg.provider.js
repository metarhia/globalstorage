...
class PostgresProvider extends StorageProvider {
  ...
  getDetails(id, fieldName, callback, permissionChecker) {
    this.getCategoryById(id, (err, leftCategory) => {
      ...
      const categorySchema = this.schema.categories.get(leftCategory);
      ...
      if (
        permissionChecker &&
        (categorySchema.catalog ||
          categorySchema.subdivision ||
          rightCategorySchema.catalog ||
          rightCategorySchema.subdivision)
      ) {
        callback(new GSError(errorCodes.INVALID_CATEGORY_TYPE));
        return;
      }
      runIf(
        permissionChecker,
        callback => {
          metasync.each(
            ...
            (categoryName, callback) => {
              permissionChecker(categoryName, null, callback);
            },
            err => {
              callback(err);
            }
  ...
  linkDetails(category, field, fromId, toIds, callback, permissionChecker) {
    ...
    const toCategorySchema = this.schema.categories.get(toCategory);
    if (
      permissionChecker &&
      (categorySchema.catalog ||
        categorySchema.subdivision ||
        toCategorySchema.catalog ||
        toCategorySchema.subdivision)
    ) {
      process.nextTick(callback, new GSError(errorCodes.INVALID_CATEGORY_TYPE));
      return;
    }
    ...
    runIf(
      ...
      callback => {
        metasync.each(
          [[category, null], [toCategory, { accessType: 'read' }]],
          (args, callback) => {
  ...
  // Log event
  log(userId, operation, { id, category, query, patch, action }) {
    this.create(
      'Log',
      {
        SystemUser: userId,
