...
class PostgresProvider extends StorageProvider {
  ...
  [uploadCategoriesAndActions](callback) {
    const categories = common.iter(this.schema.categories)
        ...
        metasync.each(
          common.iter(categories).flatMap(c => {
            const actions = getCategoryActions(
              this.schema.categories.get(c.Name).definition
            );
            actions.forEach(a => {
              a.Category = c.Id;
            });
  ...
  getDetails(id, fieldName, callback) {
    this.getCategoryById(id, (err, leftCategory) => {
      ...
      const categoryDefinition =
        this.schema.categories.get(leftCategory).definition;
      ...
      this.pool.query(
        ...
        ` WHERE ${escapedManyTableName}.${escapeIdentifier(leftCategory)} = $1`,
        [id],
        (err, res) => {
  ...
  set(obj, callback) {
    ...
    const updateRecord = (category, obj, client, callback) => {
      const categoryDefinition =
        this.schema.categories.get(category).definition;
    ...
    this.getCategoryById(obj.Id, (err, category) => {
      ...
      const error = this.schema.validateCategory(category, toValidate);
      if (error) {
        callback(new GSError(
  ...
  create(category, obj, callback) {
    const error = this.schema.validateCategory(category, obj);
    if (error) {
  ...
  update(category, query, patch, callback) {
    const error = this.schema.validateCategory(category, patch, true);
    if (error) {
      process.nextTick(callback, new GSError(
        errorCodes.INVALID_SCHEMA,
        `Invalid schema provided: ${error.toString()}`
  ...
  delete(category, query, callback) {
    const categoryDefinition =
      this.schema.categories.get(category).definition;
    // TODO: check here if the category is not referenced in another category's
    // Include decorator after metaschema is updated (required functionality
    // was added in https://github.com/metarhia/metaschema/pull/170)
    const includedCategories = extractIncludeCategories(categoryDefinition);
    const [
      deleteQuery,
      queryParams,
    ] = generateDeleteQuery(category, includedCategories, query);
  ...
  linkDetails(category, field, fromId, toIds, callback) {
    ...
    const tableName = manyToManyTableName(
      category, categoryDefinition[field].category, field
    );
    if (!Array.isArray(toIds)) {
      toIds = [toIds];
    }
    // TODO: add support for linking the records placed on different servers
    const query = `INSERT INTO ${escapeIdentifier(tableName)}` +
