...
class PostgresProvider extends StorageProvider {
  ...
  async [setBigintParser]() {
    let oid;
    try {
      ({ oid } = await this.pool.query("SELECT 'bigint'::regtype::oid"));
    } catch (err) {
      this.systemLogger(err);
      throw new GSError(errorCodes.INTERNAL_PROVIDER_ERROR, err);
    }
    pg.types.setTypeParser(oid, val => new common.Int64(val));
  }
  ...
  async set(obj, permissionChecker) {
    ...
    try {
      ...
      await this[rollback](client, err);
    } finally {
      client.release();
    }
  }
  ...
  async create(category, obj, permissionChecker) {
    ...
    try {
      ...
      return this[rollback](client, err);
    } finally {
      client.release();
    }
  }
