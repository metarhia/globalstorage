...
class StorageProvider {
  ...
  checkPermission(category, actionId, accessFlag, userId, args) {
    ...
    const user = this.permissionCache.SystemUser.get(userId);
    const categoryId = this.permissionCache.Category.get(category).Id;
    ...
    for (const { Permissions } of user.Roles) {
      ...
      const permission = Permissions.find(permission => {
        ...
        const catalog = this.permissionCache.Catalog.get(permission.Catalog);
        const subdivision =
          this.permissionCache.Subdivision.get(permission.Subdivision);
        if (
  ...
  execute(category, name, session, args, callback) {
    const categoryActions = this.schema.actions.get(category);
    if (!categoryActions) {
      callback(
        new GSError(
    ...
    const actionId = this.permissionCache.Action.get(name).Id;
    ...
    if (action.form) {
      const validationError = this.schema.validateForm(form, args);
      if (validationError) {
        callback(new GSError(
