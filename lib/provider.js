...
class StorageProvider {
        ...
        return (...args) => {
          ...
          provider[prop].call(
            ...
            (err, result) => {
              ...
              callback(LogStatus.strip(err), LogStatus.strip(result));
            },
            permissionChecker
          );
        };
  ...
  execute(category, action, actionArgs, callback, permissionChecker) {
    runIfFn(permissionChecker, category, action, err => {
      ...
      let error;
      [error, actionArgs[1]] = this.schema.createAndValidate(
        'action',
        foundAction,
        actionArgs[1]
      );
      ...
      let thenHandlers = [val => callback(null, val), callback];
      if (!this.loggingEnabled) {
        thenHandlers = thenHandlers.map(fn => val => fn(LogStatus.strip(val)));
      }
      execute(this, ...actionArgs).then(...thenHandlers);
    });
  }
