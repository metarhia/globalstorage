...
const { getCategoryType, extractDecorator } = require('./schema.utils');
...
const generateType = (domainName, domain) => {
  ...
  if (domainKind === 'Object') {
    const type = PREDEFINED_DOMAINS[domainName] ||
...
const requiresForeignKey = (categories, link, from, to) => {
  const destination = extractDecorator(categories.get(from));
  const source = extractDecorator(categories.get(to));
...
const generateManyToMany = (from, to, name, link, categories) => {
  ...
  if (requiresForeignKey(categories, link, from, to)) {
    lines.push(generateLink(
      tableName, from, from, 'Master', { category: from }
    ));
    lines.push(generateLink(tableName, to, to, 'Master', { category: to }));
  }
...
const generateLinks = (links, categories) => links
  .sort(({ link }) => extractDecorator(link) === 'Many')
...
const createHistorySchema = schema => {
  ...
  history._Creation = { domain: 'DateTime', required: true, index: true };
  history._Effective = { domain: 'DateTime', required: true, index: true };
  history._Cancel = { domain: 'DateTime', index: true };
  history._HistoryStatus = { domain: 'HistoryStatus', required: true };
  history._Identifier = {
    category: 'Identifier', required: true, index: true,
  };
  return history;
};
...
const preprocessSchema = schema => {
  ...
  for (const [name, category] of schema) {
    ...
    if (type === 'History') {
      ...
      processed.set(
        ...
        {
          name: historyName,
          definition: createHistorySchema(category.definition),
        }
      );
    }
  }
...
// Returns: <string>
const generateExtensions = names => names.map(extName =>
...
const generateDDL = schema => {
  ...
  const tablesSQL = generateTables(preprocessSchema(schema.categories), types);
