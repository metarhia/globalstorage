...
const generateLink = (from, to, name, type, link) => {
  let sql =
    `ALTER TABLE ${escapeIdentifier(from)} ADD CONSTRAINT fk${from}${name} ` +
    `FOREIGN KEY (${escapeIdentifier(name)}) `;

  sql += `REFERENCES "${link.category}" ("Id") ON UPDATE RESTRICT ON DELETE `;
  if (type === 'Object') {
...
const generateManyToMany = (from, to, name, link, categories) => {
  ...
  lines.push(
    ...
    `ADD CONSTRAINT pk${name}${from}${to} ` +
...
const generateIndexes = (indexes, table) => indexes
  .map(({ name, property: index }) => {
    ...
    return `CREATE INDEX idx${table}${name} ` +
...
const generateUnique = (unique, table) => unique
  .map(({ name, property: index }) => {
    ...
    return (
      ...
      `ADD CONSTRAINT ak${table}${name} UNIQUE (${prop});`
    );
  })
  .join('\n');
...
const generateTable = (
  ...
  const primaryKey =
    `ALTER TABLE ${escapeIdentifier(name)} ` +
    `ADD CONSTRAINT pk${name}Id PRIMARY KEY (${escapeIdentifier('Id')});`;
...
const generateTables = (schema, types) => {
  ...
  return iter(Object.keys(schema))
    .map(name => {
      ...
      const type = CATEGORY_TYPES[category.constructor.name] || 'Local';
      return {
        name,
        category,
        type,
