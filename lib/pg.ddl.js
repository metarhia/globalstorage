...
const typeFromDomain = {
  bigint: () => 'bigint',
  number: domain =>
    (domain.subtype === 'int' ? 'integer' : 'double precision'),
  string: () => 'text',
  function: () => 'text',
  boolean: () => 'boolean',
...
// Returns: boolean
const requiresForeignKey =
  (categories, from) => isLocalCategory(categories.get(from).definition);
...
const generateLink = (from, to, name, type, destination) => {
  ...
  sql += `REFERENCES "${destination}" (${escapeIdentifier('Id')}) ` +
...
const getResolvableLinks = (
  ...
  const unresolved = links.filter(
    ({ name, property: link, destination }) => {
...
const createHistorySchema = (schema, domains, categories) => {
  ...
  const dateTime = domains.get('DateTime').definition;
  history._Creation = {
    domain: 'DateTime',
    required: true,
    index: true,
      ...
      processed.set(
        historyName,
        {
          ...
          definition: createHistorySchema(
            category.definition, domains, categories
          ),
        }
      );
    }
