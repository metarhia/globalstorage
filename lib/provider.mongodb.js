...
const MongodbCursor = require('./cursor.mongodb');
...
MongodbProvider.prototype.open = function(options, callback) {
  ...
  StorageProvider.prototype.open.call(this, options, () => {
    if (this.connection) {
      this.storage = this.connection.collection('gsStorage');
      this.metadata = this.connection.collection('gsMetadata');
...
MongodbProvider.prototype.index = function(def, callback) {
  ...
  const keys = MongodbCursor.fields(def.fields);
  const options = {
