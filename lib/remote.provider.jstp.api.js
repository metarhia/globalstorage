...
const createRemoteProviderJstpApi = (gsProvider, cursorFactory) => ({
  provider: {
    ...
    set(connection, record, callback) {
      ...
      gsProvider.log(userId, 'Update', { id: record.Id, patch: record });
      gsProvider.set(
        record,
        err => {
    ...
    create(connection, category, record, callback) {
      ...
      gsProvider.log(userId, 'Insert', { category, query: record });
      gsProvider.create(
        category,
        record,
        (err, id) => {
    ...
    update(connection, category, query, patch, callback) {
      ...
      gsProvider.log(userId, 'Update', { category, query, patch });
      gsProvider.update(
        category,
        query,
        patch,
    ...
    linkDetails(connection, category, field, fromId, toIds, callback) {
      ...
      gsProvider.log(userId, 'Update', { category, id: fromId, patch: toIds });
    ...
    delete(connection, category, query, callback) {
      ...
      gsProvider.log(userId, 'Delete', { category, query });
      gsProvider.delete(
        category,
        query,
        (err, count) => {
    ...
    select(connection, category, jsql, callback) {
      ...
      gsProvider.log(userId, 'Read', { category, query: jsql });
    ...
    execute(connection, category, action, args, callback) {
      ...
      gsProvider.log(userId, null, { category, action, query: args });
      checkExecutePermission(
        gsProvider,
        category,
        action,
