...
class SelectBuilder {
  ...
  _where(key, cond, value, mod) {
    ...
    return { key, cond, value: escapeValue(value), mod };
  }

  where(key, cond, value) {
    this.operations.get('where').push(this._where(key, cond, value));
    return this;
  }

  whereNot(key, cond, value) {
    this.operations.get('where').push(this._where(key, cond, value, 'NOT'));
    return this;
  }
  ...
  _whereIn(key, conds, mod) {
    const value = `(${conds.map(escapeValue).join(', ')})`;
    return { key, cond: 'IN', value, mod };
  }

  whereIn(key, conds) {
    this.operations.get('where').push(this._whereIn(key, conds));
    return this;
  }

  whereNotIn(key, conds) {
    this.operations.get('where').push(this._whereIn(key, conds, 'NOT'));
    return this;
  }
