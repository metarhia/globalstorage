...
class RawSqlBuilder extends SqlBuilder {
  constructor(sqlTemplate, params) {
    super();
  ...
  build(params = []) {
    const sql = this.sqlTemplate(params.length || 1);
...
const checkType = (value, name, type) => {
  if (!(value instanceof SqlBuilder) && typeof value !== type) {
    throw new TypeError(
...
const makeParamValue = (cond, value, params) => {
  if (value instanceof SqlBuilder) {
    const [subquery] = value.build(params);
    return `(${subquery})`;
...
class SelectBuilder extends SqlBuilder {
  ...
  processWhere(query, clauses, params) {
    ...
    for (let i = 0; i < clauses.length; ++i) {
      ...
      query +=
        ` ${clause.cond} ` + makeParamValue(clause.cond, clause.value, params);
    }
    return query;
  }
