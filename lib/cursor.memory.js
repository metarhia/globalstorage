...
MemoryCursor.prototype.fetch = function(done) {
  ...
  this.jsql.forEach(operation => {
    const fn = MemoryCursor.operations[operation.op];
...
MemoryCursor.operations = {};

MemoryCursor.operations.select = (operation, dataset) => {
  const query = operation.query;
  const fields = Object.keys(query);
  const compare = transformations.compare;
  return dataset.filter(record => {
    let i, field, condition, keep;
    for (i = 0; i < fields.length; i++) {
      field = fields[i];
      condition = query[field];
      keep = compare(record, field, condition);
...
MemoryCursor.operations.distinct = (operation, dataset) => {
  const keys = new Set();
...
MemoryCursor.operations.order = (operation, dataset) => {
  let fields = operation.fields;
  if (typeof(operation) === 'string') fields = [fields];
  dataset.sort((r1, r2) => {
...
MemoryCursor.operations.desc = (operation, dataset) => {
  let fields = operation.fields;
  if (typeof(operation) === 'string') fields = [fields];
  dataset.sort((r1, r2) => {
    const a1 = fields.map(field => r1[field]).join('\x00');
...
MemoryCursor.operations.projection = (operation, dataset) => (
  transformations.projection(operation.fields, dataset)
);

MemoryCursor.operations.row = (operation, dataset) => (
  transformations.row(dataset)
);

MemoryCursor.operations.col = (operation, dataset) => (
  transformations.col(dataset, operation.field)
);

MemoryCursor.operations.one = (operation, dataset) => (
  dataset[0]
);

MemoryCursor.operations.union = (operation, dataset) => (
  transformations.union(dataset, operation.cursor.dataset)
);

MemoryCursor.operations.intersection = (operation, dataset) => (
  transformations.intersection(dataset, operation.cursor.dataset)
);

MemoryCursor.operations.difference = (operation, dataset) => (
  transformations.difference(dataset, operation.cursor.dataset)
);

MemoryCursor.operations.complement = (operation, dataset) => (
