...
const { SearchIndex } = require('./fts/search-index.js');
const { SearchRepositoryNode } = require('./fts/search-repository-node.js');
...
class Storage {
  ...
  async #init() {
    ...
    this.#search = await new SearchIndex(
      this,
      this.#chain,
      await new SearchRepositoryNode(this.#path),
    );
  ...
  async saveData(id, data, options = {}) {
    ...
    // Update full-text search index (skip encrypted records)
    if (!encrypted && this.#search) await this.#search.index(id, data);
  ...
  async delete(id) {
    ...
    if (exists) {
      ...
      if (this.#search) await this.#search.remove(id);
